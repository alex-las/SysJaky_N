...
public class AltchaService : IAltchaService
{
    ...
    public bool Verify(AltchaProof proof)
    {
        ...
        // 3) proof-of-work – dovol obì varianty (bez dvojteèky i s dvojteèkou) a ovìø jak "hex nuly", tak "bitové nuly"
        ...
        // Akceptuj, pokud alespoò jedna varianta projde – nejprve "hex nuly", pak "bitové nuly"
        if (HasLeadingZeroHex(h1, difficulty) || HasLeadingZeroHex(h2, difficulty)) return true;

        // Pokud tvùj build Altchy interpretuje difficulty jako "bitové nuly" (nikoli hex),
        // projde jedna z tìchto variant:
